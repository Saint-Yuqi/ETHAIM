...
from src.data.dataset import EmbeddingPairsDataset, load_test_examples
...
def evaluate_model_on_test(model: SentenceTransformer, cfg, test_df: pd.DataFrame) -> Tuple[float, float]:
    """Evaluate model on test set using the same logic as eval_e5_offline.py"""

    ranks = []
    hits = []

    for _, group in test_df.groupby(cfg.dataset.fields.query):
        if group.empty:
            continue
...
def main(cfg: DictConfig) -> None:
    ...
    unique_queries = test_df[cfg.dataset.fields.query].nunique()
    print(f"Test set: {len(test_df):,} examples, {unique_queries} unique queries")
    ...
    avg_rank_base, hit_base = evaluate_model_on_test(base_model, cfg, test_df)
    ...
    try:
        ...
        avg_rank_ft, hit_ft = evaluate_model_on_test(ft_model, cfg, test_df)
        print(f"[Finetuned] avg_rank_best = {avg_rank_ft:.4f}, Hit@K = {hit_ft:.4f}")

        # Show improvement
        rank_improvement = avg_rank_base - avg_rank_ft
        hit_improvement = hit_ft - hit_base
        print(f"\nImprovement: rank ↓{rank_improvement:.4f}, hit ↑{hit_improvement:.4f}")

    except Exception as e:
        print(f"Could not load fine-tuned model from {finetuned_path}: {e}")
